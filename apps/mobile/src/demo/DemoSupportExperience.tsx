...
const projects = ['Amaravati Solar Commons', 'Aarohan Medical City', 'Account & access'] as const;
...
export function DemoSupportExperience({ state, onAction }: Props) {
  ...
  const close = () => { setTicketOpen(false); setErrors(emptyErrors); submitting.current = false; };
  ...
  return <View style={styles.page} testID="customer-support-page">
    ...
    <View style={styles.content}>
      ...
      <View style={styles.actionStack}>
        <SupportCard eyebrow="AVAILABLE NOW" icon="◉" onPress={openLiveChat} subtitle="Message a Karaa support specialist" title="Live Chat" />
        <SupportCard eyebrow="WE’LL FOLLOW UP" icon="＋" onPress={openTicketForm} subtitle="Share an issue with project context" title="Raise a Ticket" />
      </View>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.scrim}>
        <View accessibilityLabel="Raise a support ticket" accessibilityViewIsModal style={styles.modal} testID="support-ticket-modal">
          ...
          <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            <FieldLabel text="PROJECT CONTEXT" />
            <View accessibilityRole="radiogroup" style={styles.options}>{projects.map((item) => <Option key={item} label={item} onPress={() => setProject(item)} selected={project === item} />)}</View>
            {errors.project && <ErrorText text="Choose a project context." />}
            <FieldLabel text="CATEGORY" />
            <View accessibilityRole="radiogroup" style={styles.options}>{categories.map((item) => <Option key={item} label={item} onPress={() => setCategory(item)} selected={category === item} />)}</View>
...
function SupportCard({ eyebrow, icon, onPress, subtitle, title }: { eyebrow: string; icon: string; onPress: () => void; subtitle: string; title: string }) {
...
const styles = StyleSheet.create({
  page: { backgroundColor: '#F8F5EE', marginHorizontal: -16, marginTop: -16 },
  hero: { backgroundColor: '#EEE8DC', height: 218, overflow: 'hidden' }, heroCopy: { maxWidth: '67%', paddingLeft: 24, paddingTop: 38, zIndex: 2 }, heroImage: { bottom: 0, height: 218, opacity: .83, position: 'absolute', right: -38, width: '62%' },
  ...
  supportCard: { alignItems: 'center', backgroundColor: '#FFFEFA', borderColor: '#DED9CF', borderRadius: 12, borderWidth: 1, flexDirection: 'row', minHeight: 112, paddingHorizontal: 18, paddingVertical: 16 }, cardIcon: { alignItems: 'center', backgroundColor: '#F3E9D5', borderRadius: 25, height: 50, justifyContent: 'center', width: 50 }, cardIconText: { color: gold, fontSize: 22, fontWeight: '700' }, cardCopy: { flex: 1, marginHorizontal: 14 }, cardEyebrow: { color: gold, fontSize: 8, fontWeight: '900', letterSpacing: .8 }, cardTitle: { color: '#171815', fontFamily: 'serif', fontSize: 20, marginTop: 3 }, cardSubtitle: { color: '#6B6A64', fontSize: 11, lineHeight: 16, marginTop: 2 }, cardArrow: { color: gold, fontSize: 22 },
  historyHeading: { alignItems: 'flex-end', borderTopColor: '#DED9CF', borderTopWidth: 1, flexDirection: 'row', justifyContent: 'space-between', paddingTop: 20 }, count: { color: gold, fontFamily: 'serif', fontSize: 24 }, ticketList: { gap: 10 }, ticketRow: { alignItems: 'center', backgroundColor: '#FFFEFA', borderColor: '#E0DCD3', borderRadius: 10, borderWidth: 1, flexDirection: 'row', minHeight: 112, paddingHorizontal: 14, paddingVertical: 13 }, ticketMark: { alignItems: 'center', backgroundColor: '#191A17', borderRadius: 20, height: 40, justifyContent: 'center', width: 40 }, ticketMarkText: { color: '#D9A64B', fontFamily: 'serif', fontSize: 17 }, ticketCopy: { flex: 1, gap: 2, marginLeft: 12 }, ticketTop: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, ticketId: { color: gold, fontSize: 9, fontWeight: '900', letterSpacing: .5 }, status: { color: '#8A5D12', fontSize: 8, fontWeight: '900' }, resolved: { color: '#397445' }, ticketSubject: { color: '#1D1E1B', fontSize: 14, fontWeight: '800', lineHeight: 18 }, ticketContext: { color: '#666760', fontSize: 10 }, ticketUpdate: { color: '#929089', fontSize: 9, marginTop: 3 }, chevron: { color: gold, fontSize: 25, marginLeft: 8 },
  scrim: { alignItems: 'center', backgroundColor: 'rgba(15,15,13,.66)', flex: 1, justifyContent: 'center', padding: 16 }, modal: { backgroundColor: '#FFFEFA', borderRadius: 16, maxHeight: '92%', maxWidth: 480, overflow: 'hidden', width: '100%' }, modalHeader: { alignItems: 'center', borderBottomColor: '#E5E0D7', borderBottomWidth: 1, flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16 }, modalEyebrow: { color: gold, fontSize: 9, fontWeight: '900', letterSpacing: 1 }, modalTitle: { color: '#171815', fontFamily: 'serif', fontSize: 25 }, close: { alignItems: 'center', justifyContent: 'center', minHeight: 44, minWidth: 44 }, closeText: { color: '#171815', fontSize: 29, fontWeight: '300' }, form: { padding: 20, paddingBottom: 28 }, label: { color: '#54554F', fontSize: 9, fontWeight: '900', letterSpacing: .8, marginBottom: 7, marginTop: 14 }, options: { gap: 7 }, priorityRow: { flexDirection: 'row', gap: 8 }, option: { alignItems: 'center', backgroundColor: '#F8F5EE', borderColor: '#DED9CF', borderRadius: 8, borderWidth: 1, flexDirection: 'row', flexGrow: 1, minHeight: 44, paddingHorizontal: 12 }, optionSelected: { backgroundColor: '#FFF8E9', borderColor: gold }, radio: { borderColor: '#A3A098', borderRadius: 6, borderWidth: 1, height: 12, marginRight: 8, width: 12 }, radioSelected: { backgroundColor: gold, borderColor: gold }, optionText: { color: '#575851', fontSize: 11 }, optionTextSelected: { color: '#171815', fontWeight: '800' }, input: { backgroundColor: '#F8F5EE', borderColor: '#DED9CF', borderRadius: 8, borderWidth: 1, color: '#171815', fontSize: 13, minHeight: 48, paddingHorizontal: 12, paddingVertical: 10 }, textarea: { minHeight: 96 }, inputError: { borderColor: '#A33B32' }, error: { color: '#A33B32', fontSize: 10, fontWeight: '700', marginTop: 4 }, modalActions: { flexDirection: 'row', gap: 9, marginTop: 22 }, cancel: { alignItems: 'center', borderColor: '#CFC9BE', borderRadius: 8, borderWidth: 1, flex: 1, justifyContent: 'center', minHeight: 48 }, cancelText: { color: '#1B1C19', fontSize: 12, fontWeight: '900' }, submit: { alignItems: 'center', backgroundColor: '#171815', borderRadius: 8, flex: 1.5, justifyContent: 'center', minHeight: 48 }, submitText: { color: '#FFFEFA', fontSize: 12, fontWeight: '900' },
});
