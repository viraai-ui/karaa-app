...
import { Image, Pressable, StyleSheet, Text, TextInput, useWindowDimensions, View, type ViewStyle } from 'react-native';
...
export function DemoAppBar({ role, onSwitchWorkspace, onOpenSearch, onOpenNotifications, healthcareBack }: { role: OfflineDemoRole; onSwitchWorkspace: () => void; onOpenSearch: () => void; onOpenNotifications: () => void; healthcareBack?: () => void }) {
  ...
  const compactEmployeeHeader = role === 'employee' && width <= 340;
  return (
    <View style={[styles.appBar, role === 'employee' && styles.employeeAppBar, healthcareBack && styles.healthcareAppBar, { paddingTop: insets.top }]} testID="demo-app-bar">
      <View style={styles.brandGroup}>
        ...
        <KaraaBrand height={healthcareBack ? 28 : role === 'employee' ? compactEmployeeHeader ? 23 : 37 : 26} variant={healthcareBack ? 'crown' : 'lockup'} />
      </View>
      <View style={[styles.topActions, role === 'employee' && styles.employeeTopActions, compactEmployeeHeader && styles.compactEmployeeTopActions]}>
...
const styles = StyleSheet.create({
  ...
  employeeAppBar: { minHeight: 77, paddingHorizontal: 16 },
  ...
  topActions: { alignItems: 'center', flexDirection: 'row', flexShrink:0 }, employeeTopActions: { justifyContent: 'space-between', marginRight: -5, width: 153 }, compactEmployeeTopActions: { width: 133 }, iconButton: { alignItems: 'center', height: 44, justifyContent: 'center', position: 'relative', width: 44 }, topIcon: { color: colors.paper, fontSize: 24, fontWeight: '300' }, notificationDot: { backgroundColor: '#E89A0A', borderColor: '#050605', borderRadius: 4, borderWidth: 1.5, height: 7, position: 'absolute', right: 8, top: 8, width: 7 }, avatarTarget: { alignItems: 'center', height: 44, justifyContent: 'center', width: 44 }, avatar: { alignItems: 'center', backgroundColor: '#474845', borderColor: '#686963', borderRadius: radii.pill, borderWidth: 1, height: 38, justifyContent: 'center', width: 38 }, avatarText: { color: colors.paper, fontSize: 10, fontWeight: '900' },
  ...
  bottomNavigation: { ...cardShadow, backgroundColor: '#FFFFFF', borderColor: '#E3E1DC', borderTopLeftRadius: 16, borderTopRightRadius: 16, borderWidth: 1, flexDirection: 'row', marginHorizontal: 8, minHeight: NAVIGATION_BASE_HEIGHT, paddingHorizontal: 2, paddingTop: 12 }, tabWrapper: { flex: 1, minWidth: 0 }, tab: { alignItems: 'center', flex: 1, gap: 2, justifyContent: 'center', minHeight: 44, minWidth: 44 }, navIconBox: { alignItems: 'center', height: 25, justifyContent: 'center', width: 25 },tabSelected: { backgroundColor: 'rgba(181, 138, 58, 0.10)', borderRadius: radii.sm }, tabIcon: { color: '#D6D3CD', fontSize: 21, fontWeight: '500', lineHeight: 23 }, tabLabel: { color: '#171717', fontSize: 10, fontWeight: '600', textAlign: 'center' },referenceTabLabel: { color: CUSTOMER_NAV_INACTIVE, marginTop: 1 }, tabActive: { color: colors.brass },

  customerBottomNavigation: { minHeight: NAVIGATION_BASE_HEIGHT }, customerTabLabel: { color: '#171717', fontSize: 10, fontWeight: '500' }, activeRail: { backgroundColor: '#B58A29', height: 2, left: 8, position: 'absolute', right: 8, top: -7 },
