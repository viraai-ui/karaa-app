...
export function DemoUtilitySheet({ mode, onDismiss }: { mode: 'search' | 'notifications'; onDismiss: () => void }) {
  ...
  return <View accessibilityLabel={search ? 'Search panel' : 'Notifications panel'} accessibilityViewIsModal style={styles.sheetBackdrop}>
    <MotionModalEntrance style={styles.utilitySheet}>
      ...
      {search ? <><DemoSearchField accessibilityLabel="Search projects and tenders" onChangeText={setQuery} placeholder="Search projects and tenders" value={query} /><Text accessibilityLiveRegion="polite" style={styles.utilityCopy}>{query.trim() ? `No saved demo results for “${query.trim()}”.` : 'Enter a project or tender name. This demo searches only saved showcase content.'}</Text></> : <View accessible accessibilityLabel="Unread: Aarohan Medical City field update is ready for review" style={styles.noticeCard}><Text style={styles.noticeUnread}>UNREAD</Text><Text style={styles.noticeTitle}>Field update ready for review</Text><Text style={styles.utilityCopy}>Aarohan Medical City · saved demo activity</Text></View>}
    </MotionModalEntrance>
  </View>;
}
...
export function DemoImageFrame({ source, accessibilityLabel, caption = 'Demo visual', ratio = 16 / 9, height, width }: { source: number; accessibilityLabel: string; caption?: string; ratio?: number; height?: number; width?: number }) {
  ...
  return <View style={styles.imageBlock}><View accessible accessibilityLabel={accessibilityLabel} accessibilityRole="image" style={[styles.imageFrame, frameSize]}><MotionImage resizeMode="cover" source={source} style={styles.image} /></View><Text numberOfLines={1} style={styles.imageCaption}>{caption}</Text></View>;
}
