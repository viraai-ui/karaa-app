...
export function DemoBottomNavigation({ tabs, selectedTab, onSelect, role }: { tabs: readonly OfflineDemoTab[]; selectedTab: OfflineDemoTabKey; onSelect: (tab: OfflineDemoTabKey) => void; role?: OfflineDemoRole }) {
  const insets = useSafeAreaInsets();
  return <View accessibilityRole="tablist" style={[styles.bottomNavigation, { paddingBottom: 8 + insets.bottom }]} testID="demo-bottom-navigation">{tabs.map((tab) => { const selected = selectedTab === tab.key; const color = selected ? colors.brass : CUSTOMER_NAV_INACTIVE; return <MotionPressable accessibilityLabel={tab.label} accessibilityRole="tab" accessibilityState={{ selected }} key={tab.key} onPress={() => onSelect(tab.key)} style={[styles.tab, selected && styles.tabSelected]} wrapperStyle={styles.tabWrapper}>{role === 'customer' ? <CustomerNavIcon color={color} tabKey={tab.key} /> : <Text style={[styles.tabIcon, selected && styles.tabActive]} testID={`role-nav-icon-${tab.key}`}>{iconGlyphs[tab.icon]}</Text>}<Text numberOfLines={1} style={[styles.tabLabel, role === 'customer' && styles.customerTabLabel, selected && styles.tabActive]}>{tab.label}</Text></MotionPressable>; })}</View>;
}
...
export function OfflineDemoPrimitivesPreview() {
  return <View><DemoAppBar role="employee" onSwitchWorkspace={() => undefined} /><DemoBottomNavigation onSelect={() => undefined} role="employee" selectedTab="power" tabs={offlineRoleTabs.employee} /></View>;
}
...
const styles = StyleSheet.create({
  ...
  bottomNavigation: { backgroundColor: '#050605', flexDirection: 'row', minHeight: 75, paddingBottom: 8, paddingHorizontal: 2, paddingTop: 7 }, tabWrapper: { flex: 1, minWidth: 0 }, tab: { alignItems: 'center', flex: 1, gap: 3, justifyContent: 'center', minHeight: 58, minWidth: 44 }, tabSelected: { backgroundColor: 'rgba(181, 138, 58, 0.10)', borderRadius: radii.sm }, tabIcon: { color: '#D6D3CD', fontSize: 21, fontWeight: '500', lineHeight: 23 }, tabLabel: { color: '#D6D3CD', fontSize: 8, fontWeight: '700', textAlign: 'center' }, customerTabLabel: { color: CUSTOMER_NAV_INACTIVE, marginTop: 1 }, tabActive: { color: colors.brass },
