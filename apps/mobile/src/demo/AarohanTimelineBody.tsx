...
export function AarohanTimelineBody({
  project,
  portfolio,
  selectedTab,
  onAction,
  ...
  return (
    <View style={a.page} testID="aarohan-timeline-body">
      <Pressable
        testID="aarohan-back-control"
        accessibilityRole="button"
        accessibilityLabel={`Back to ${portfolio.title}`}
        accessibilityHint="Returns to the Multi-Specialty Hospitals projects"
        onPress={() => onAction({ type: "return-to-subvertical" })}
        style={a.back}
      >
        <Text style={a.backArrow}>‹</Text>
        <Text style={a.backLabel}>MULTI-SPECIALTY HOSPITALS</Text>
      </Pressable>
