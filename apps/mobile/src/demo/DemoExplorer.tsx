...
function DashboardContinuation({ onAction, random = Math.random }: { onAction: (action: OfflineDemoAction) => void; random?: () => number }) {
  ...
  return <View style={styles.continuation} testID="customer-dashboard-continuation">
    ...
    <View style={styles.portfolioSyncRow}><Text style={styles.portfolioSyncText}>✓  Synced with My Portfolio</Text><Text style={styles.portfolioSyncUpdates}>{customerPortfolioProjects.filter(project => project.fresh).length} new updates</Text></View>
