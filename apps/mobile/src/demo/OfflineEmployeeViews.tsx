...
export const EMPLOYEE_TASK_VISUAL_METRICS = { supportedWidths: [320, 360, 390, 480] as const, minimumTarget: 44, pageGutter: 21, packageHeight: 203, filterWidths: [52, 78, 72, 95] as const, taskRowHeight: 62, taskCount: 7, footerHeight: 89 };
...
function WorkView({ state, onAction }: Pick<Props, 'state' | 'onAction'>) {
  ...
  return <View style={styles.taskPage} testID="employee-my-tasks">
    ...
    <Pressable accessibilityHint="Opens the existing field progress review workflow" accessibilityLabel="Record progress update" accessibilityRole="button" onPress={() => onAction({ type: 'open-field-review' })} style={styles.packageCard} testID="active-package-card">
      <Image accessibilityElementsHidden source={require('../../assets/brand/karaa-crown.webp')} style={styles.packageMotif} />
    ...
    <View accessibilityRole="tablist" style={styles.filters}>{(['All','Pending','Upload','Completed'] as TaskFilter[]).map((item, index) => <Pressable accessibilityRole="tab" accessibilityState={{ selected: filter === item }} key={item} onPress={() => setFilter(item)} style={[styles.filterTarget, { width: EMPLOYEE_TASK_VISUAL_METRICS.filterWidths[index] }, filter === item && styles.filterSelected]}><Text style={[styles.filterText, filter === item && styles.filterTextSelected]}>{item}</Text></Pressable>)}</View>
    ...
    <View style={styles.submissionGrid}><SubmissionCard image={require('../../assets/demo/submission-inverter-cabinets.webp')} label="2 photos" title="Inverter cabinet photos" time="Today  ·  08:45 AM" /><SubmissionCard image={require('../../assets/demo/submission-solar-crew.webp')} label="Checklist" title="Alignment checklist" time="Today  ·  08:30 AM" />{showAll ? <SubmissionCard image={demoVisualAssets.progress.source} label="Site note" title="Row commissioning note" time="Yesterday  ·  05:20 PM" /> : null}</View>
...
function SubmissionCard({ image, label, title, time }: { image: number; label: string; title: string; time: string }) { return <View style={styles.submissionCard}><View style={styles.submissionImageWrap}><Image accessibilityLabel={`${title} submission image`} source={image} style={styles.submissionImage} /><View style={styles.mediaBadge}><Feather color="#282521" name={label === '2 photos' ? 'camera' : 'clipboard'} size={10} /><Text style={styles.mediaBadgeText}>{label}</Text></View></View><Text numberOfLines={1} style={styles.submissionName}>{title}</Text><View style={styles.submissionMeta}><Text numberOfLines={1} style={styles.submissionTime}>{time}</Text><View style={styles.submittedPill}><Text style={styles.submittedText}>Submitted</Text></View></View></View>; }
function CalendarGlyph() { return <Feather color="#77736B" name="calendar" size={11} />; }
function UploadGlyph() { return <Feather color="#A37B24" name="upload-cloud" size={21} />; }
function TaskGlyph({ name }: { name: string }) { const icon = ({ camera:'camera-outline', tag:'tag-outline', clipboard:'clipboard-outline', note:'note-edit-outline', search:'magnify', shield:'shield-outline', tool:'wrench-outline' } as const)[name as 'camera']; return <MaterialCommunityIcons color="#34302A" name={icon} size={23} />; }
...
const styles = StyleSheet.create({
  taskPage: { gap: 0 },
  taskEyebrow: { color: '#A67D27', fontSize: 10, fontWeight: '900', letterSpacing: 1.15 },
  taskTitle: { color: '#171512', fontFamily: 'serif', fontSize: 46, fontWeight: '400', lineHeight: 51, marginBottom: 14, marginTop: 3 },
  packageCard: { backgroundColor: '#080808', borderRadius: 14, height: 203, overflow: 'hidden', paddingBottom: 15, paddingHorizontal: 17, paddingTop: 16 },
  packageMotif: { bottom: -10, height: 152, opacity: .06, position: 'absolute', resizeMode: 'contain', right: -37, width: 145 },
