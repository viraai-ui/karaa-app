...
function TenderBoard({ onAction, onOpen, state, customer }: Pick<Props, 'onAction' | 'state'> & {onOpen:(id:string)=>void;customer:boolean}) {
  ...
  return (
    <View style={styles.tendersPage} testID="tenders-page">
      <View style={styles.tenderHero}><View style={styles.heroCopy}><Text style={styles.tenderEyebrow}>OPPORTUNITIES &amp; PROCUREMENT</Text><Text style={styles.tenderTitle}>Tenders</Text><Text style={styles.tenderSubtitle}>Track, manage and monitor every tender opportunity.</Text></View><View style={styles.heroButtons}><SquareButton light={customer} label="Global search" icon="search" onPress={() => searchRef.current?.focus()} /><SquareButton light={customer} label="Open tender filters" icon="filter" onPress={() => setPanel('filters')} /></View></View>
      <View style={[styles.overviewCard,customer&&styles.customerOverviewCard]}><View style={styles.overviewHead}><Text style={[styles.overviewTitle,customer&&styles.customerDarkText]}>Tender overview</Text></View><View style={styles.metricGrid}>{overviewMetrics.map((metric,index) => <View key={metric.label} style={[styles.metric,customer&&styles.customerMetric,index===3&&styles.metricLast]}><Text style={[styles.metricValue,customer&&styles.customerDarkText]}>{metric.value}</Text><Text style={[styles.metricLabel,customer&&styles.customerMutedText]}>{metric.label}</Text></View>)}</View><View style={[styles.totalLine,customer&&styles.customerTotalLine]}><View style={styles.totalGroup}><View><Text style={[styles.totalValue,customer&&styles.customerDarkText]}>46</Text><Text style={[styles.totalLabel,customer&&styles.customerMutedText]}>Total tenders</Text></View></View><Pressable accessibilityRole="button" accessibilityLabel="View analytics" onPress={() => setPanel('analytics')} style={({pressed})=>[styles.linkTarget,pressed&&styles.pressed]}><Text style={styles.cardLink}>View analytics</Text><LineIcon name="arrow" light/></Pressable></View></View>
...
export const TENDER_BOARD_VISUAL_METRICS = Object.freeze({
  overview: { maxHeight: 170, paddingHorizontal: 12, paddingTop: 10, radius: 4 },
...
const styles = StyleSheet.create({
  ...
  showcaseTop:{alignItems:'center',backgroundColor:'#080907',flexDirection:'row',height:48,paddingHorizontal:2}, customerShowcaseTop:{backgroundColor:'#FFFFFF',borderBottomColor:'#DDD8CE',borderBottomWidth:1},
  ...
  tenderEyebrow: { color: '#9B7427', fontSize: 9, fontWeight: '800', letterSpacing: .9 },
  ...
  iconCanvas:{alignItems:'center',height:16,justifyContent:'center',position:'relative',width:16},iconMonogram:{color:'#9B7427',fontSize:8,fontWeight:'800',letterSpacing:.2},iconMonogramLight:{color:'#F5F0E6'},primitiveLine:{backgroundColor:'#9B7427',height:1.5,position:'absolute'},primitiveLineLight:{backgroundColor:'#F5F0E6'},primitiveBorderLight:{borderColor:'#F5F0E6'},searchCircle:{borderColor:'#9B7427',borderRadius:5,borderWidth:1.5,height:9,left:2,position:'absolute',top:2,width:9},searchHandle:{left:10,top:11,transform:[{rotate:'45deg'}],width:5},lineTop:{left:2,top:3,width:12},lineMiddle:{left:2,top:7,width:12},lineBottom:{left:2,top:11,width:12},lineMiddleShort:{left:4,top:7,width:8},lineBottomTiny:{left:6,top:11,width:4},sortStem:{height:12,left:7,top:2,width:1.5},sortHead:{left:4,top:4,transform:[{rotate:'45deg'}],width:5},sortFoot:{left:7,top:10,transform:[{rotate:'45deg'}],width:5},arrowShaft:{left:2,top:7,width:11},arrowHead:{borderRightColor:'#9B7427',borderRightWidth:1.5,borderTopColor:'#9B7427',borderTopWidth:1.5,height:6,left:8,position:'absolute',top:4,transform:[{rotate:'45deg'}],width:6},calendarBox:{borderColor:'#9B7427',borderRadius:1,borderWidth:1.5,height:12,left:2,position:'absolute',top:2,width:12},calendarRule:{left:3,top:6,width:10},checkShort:{left:2,top:9,transform:[{rotate:'45deg'}],width:5},checkLong:{left:5,top:7,transform:[{rotate:'-45deg'}],width:9},
  overviewCard: { backgroundColor: '#292A26', borderRadius: 4, justifyContent:'space-between', maxHeight:170, minHeight:150, overflow: 'hidden', paddingHorizontal: 12, paddingTop: 10 },
  customerOverviewCard:{backgroundColor:'#FFFFFF',borderColor:'#DDD8CE',borderWidth:1}, customerDarkText:{color:'#171713'}, customerMutedText:{color:'#77736C'}, customerMetric:{borderRightColor:'#DDD8CE'}, customerTotalLine:{borderTopColor:'#DDD8CE'}, overviewHead: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, overviewTitle: { color: '#F4EFE5', fontFamily: 'serif', fontSize: 15 }, dotsIcon:{flexDirection:'row',gap:3},
  metricGrid: { flexDirection: 'row', marginTop: 8 }, metric: { alignItems:'center',borderRightColor: '#494A45', borderRightWidth: 1, flex: 1, gap: 1 },metricLast:{borderRightWidth:0}, metricValue: { color: '#F5F0E6', fontFamily: 'serif', fontSize: 19 }, metricLabel: { color: '#BDBAB2', fontSize: 9 }, totalLine: { alignItems: 'center', borderTopColor: '#494A45', borderTopWidth: 1, flexDirection: 'row', justifyContent: 'space-between', marginTop: 8, minHeight:44 },totalGroup:{alignItems:'center',flexDirection:'row'}, totalValue: { color: '#F5F0E6', fontFamily: 'serif', fontSize: 15 }, totalLabel: { color: '#A9A69F', fontSize: 9 }, cardLink: { color: '#C7A253', fontSize: 10, fontWeight: '700' },linkTarget:{alignItems:'center',flexDirection:'row',justifyContent:'center',minHeight:44},
  ...
  chipRail: { flexDirection: 'row', gap: 4 }, chip: { alignItems: 'center', borderColor: '#D5D0C6', borderRadius: 16, borderWidth: 1, flex: 1, height: 28, justifyContent: 'center' }, chipActive: { backgroundColor: '#111210', borderColor: '#111210' }, customerChipActive:{backgroundColor:'#F4E8C9',borderColor:'#B68B24'}, chipLabel: { color: '#69665F', fontSize: 10, fontWeight: '700' }, chipLabelActive: { color: '#F7F2E8' }, customerChipLabelActive:{color:'#7C5B18'},
  ...
  appliedCard: { backgroundColor: 'transparent', borderTopColor: '#DDD8CE', borderTopWidth: 1 }, progressRow: { alignItems:'center', borderBottomColor:'#E2DDD3', borderBottomWidth:1, flexDirection:'row', minHeight:54,paddingVertical:4 },lastRow:{borderBottomWidth:1}, appliedIcon:{alignItems:'center',height:32,justifyContent:'center',marginRight:8,width:32}, progressCopy:{flex:1,gap:1,minWidth:0}, progressHeading: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, progressName: { color: '#2B2B27', flex: 1, fontSize: 12, fontWeight: '700' }, progressOwner:{color:'#858078',fontSize:10}, progressPercent: { color: '#3A3934', fontSize: 9, fontWeight: '800',marginLeft:4 }, progressBottom:{alignItems:'center',flexDirection:'row',gap:7}, progressStage:{color:'#8E6B29',fontSize:9,maxWidth:150}, slimRail: { backgroundColor: '#E6E1D7', flex:1, height: 2, overflow: 'hidden' }, slimFill: { backgroundColor: '#B58A36', height: 2 },
  ...
  detailContext: { backgroundColor: colors.ink, gap: 3, padding: spacing.md }, customerDetailContext:{backgroundColor:'#FFFFFF',borderBottomColor:'#DDD8CE',borderBottomWidth:1},
