...
export function DemoTenderExperience({ onAction, role, state }: Props) {
  ...
  return <View style={{width:'100%'}}><View style={localTenderId ? {display:'none'} : undefined}><TenderBoard customer={role === 'customer'} onAction={onAction} onOpen={setLocalTenderId} state={state} /></View>{localTenderId ? <TenderShowcase id={localTenderId} onBack={() => setLocalTenderId(null)} /> : null}</View>;
}
