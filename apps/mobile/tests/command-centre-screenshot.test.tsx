...
describe('management Overview', () => {
  ...
  it('keeps snapshot values on one line at the 27px display size',()=>{const r=screen(); const value=r.getByText('₹1,248 Cr'); expect(value.props.numberOfLines).toBe(1); expect(value.props.adjustsFontSizeToFit).toBe(true); expect(StyleSheet.flatten(value.props.style).fontSize).toBe(27);});
  it('assigns blockers and keeps activity and tender cards free of helper copy',()=>{const r=screen(); fireEvent.press(r.getByRole('button',{name:'Assign Land title clearance'})); expect(r.getByText('Assigned')).toBeTruthy(); fireEvent.press(r.getByRole('button',{name:'Open Envelope works'})); fireEvent.press(r.getByRole('button',{name:'Open activity RFI closed'})); fireEvent.press(r.getByRole('button',{name:'Open tender Interior Fit-out Works'})); expect(r.queryByText(/locally displayed|Activity detail|prototype data/)).toBeNull();});
  it('keeps blocker copy and actions in separate responsive rows',()=>{const r=screen(); const cards=r.getAllByTestId('management-blocker-card'); expect(cards).toHaveLength(3); cards.forEach(card=>{const outer=StyleSheet.flatten(card.props.style); expect(outer.flexDirection).toBeUndefined(); expect(outer.flexWrap).toBeUndefined();}); const assign=r.getByRole('button',{name:'Assign Specialist contractor shortage'}); expect(StyleSheet.flatten(assign.props.style)).toMatchObject({flexShrink:0,minHeight:44,minWidth:72});});
